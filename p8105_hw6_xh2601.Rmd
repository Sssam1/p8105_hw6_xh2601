...
The bootstrap distribution of the beta ratio shows a sharp peak near 0, with extremely long tails on both sides of the density plot. The long-tailed shape occurs because precipitation (prcp) takes the value zero on most days in the original dataset, so many bootstrap samples contain little or no variation in prcp. In such samples, the estimated coefficient beta2 for prcp approaches zero, causing the ratio of beta1 / beta2 to explode to extremely large absolute values.

To visualize the central behavior of the ratio, the following histogram looks at only the middle 98% of the bootstrap distribution (the 1%–99% interval). Within this trimmed range, the majority of values cluster around a central peak, while the tails remain wide. This reflects instability in the denominator beta2, and the ratio is not a meaningful or interpretable relationship between the predictors tmax and prcp in this sample.
...
# make comparison in table
cv_df |> 
  select(starts_with("rmse")) |> 
  summarize(
    proposed_model = mean(rmse_bwt),
    model_1 = mean(rmse_1),
    model_2 = mean(rmse_2),
  ) |> 
  knitr::kable(caption = "Mean RMSE for Three Models")
